...
export default function HowItWorks() {
  ...
  useEffect(() => {
    ...
    const draw = (progress) => {
      ...
      // Draw floating particles along the path
    ...
    const animate = () => {
      ...
      // Ease out
      progressRef.current = rawProgress < 1
  ...
  return (
    <section style={{ background: '#0f1a0d', padding: '100px 40px', position: 'relative', overflow: 'hidden' }}>
      ...
      <div style={{ maxWidth: '1200px', margin: '0 auto', position: 'relative', zIndex: 1 }}>
        ...
        {/* Animated Canvas */}
        <canvas ref={canvasRef} style={{ width: '100%', height: '160px', display: 'block', marginBottom: '24px' }} />

        {/* Step Cards */}
        <div className="steps-grid" style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '20px', marginTop: '8px' }}>
          {steps.map((step, i) => (
            <div key={i}
              style={{ background: 'rgba(255,255,255,0.04)', border: `1px solid ${step.color}30`, borderRadius: '16px', padding: '24px 20px', transition: 'all 0.3s', textAlign: 'center' }}
              ...
              {/* Step number badge */}
              <div style={{ display: 'inline-flex', alignItems: 'center', justifyContent: 'center', background: step.bg, border: `1px solid ${step.color}40`, borderRadius: '10px', width: '44px', height: '44px', color: step.color, fontWeight: '900', fontSize: '15px', marginBottom: '16px' }}>
                {step.number}
              </div>
              ...
              {/* Bottom accent */}
              <div style={{ marginTop: '16px', height: '2px', width: '32px', background: step.color, borderRadius: '2px', margin: '16px auto 0' }} />
            </div>
          ))}
        </div>
